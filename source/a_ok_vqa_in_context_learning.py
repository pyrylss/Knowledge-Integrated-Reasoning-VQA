...
def val_in_context_learning_a_ok_vqa_with_beit(llama_model, 
  ...
  with open('/scratch/project_462000472/pyry/prophet/PromptCAP/aokvqa_val_captions.json', 'r') as file:
            pc_captions_val = json.load(file)
  
  qid_to_caption_val = {entry["question_id"]: entry["pred_caption"] for entry in pc_captions_val}

  with open('/scratch/project_462000472/pyry/prophet/PromptCAP/aokvqa_train_captions.json', 'r') as file:
      pc_captions_train = json.load(file)
  
  qid_to_caption_train = {entry["question_id"]: entry["caption"] for entry in pc_captions_train}
  ...
  for i in tqdm(range(val_annotations_df.shape[0])):
    ...
    for k in range(k_ensemble): # we use k promts for each test sample
      ...
      for ni in range(n_shots_var):
        ...
        pc_caption_train = qid_to_caption_train.get(context_key.question_id, None) ###

        context_key_captions_w_knowledge = [c["caption"] for c in context_key_captions_w_knowledge]
        #context_key_captions_w_knowledge.append(pc_caption_train)
        ...
        #context_key_captions = list(context_key_captions) ###
        #context_key_captions.append(pc_caption_train)###
        #context_key_captions.extend(context_key_captions_w_knowledge)###
        ...
        captions_to_use = [
            context_key_captions_w_knowledge[i % len(context_key_captions_w_knowledge)] 
            for i in range(NO_OF_CAPTIONS_AS_CONTEXT)
        ]
        ...
        for j,caption in enumerate(context_key_captions[:NO_OF_CAPTIONS_AS_CONTEXT]):
          ...
          if j < NO_OF_CAPTIONS_AS_CONTEXT-1:
           ...
           # prompt += '%s\n%s\n%s\nEnd of Context\n'%(caption, context_key_captions_w_knowledge[0], pc_caption_train)###
            prompt += '%s\n%s\nEnd of Context\n'%(caption, pc_caption_train)
            ###prompt += '%s\nEnd of Context\n'%caption
      ...
      pc_caption_test = qid_to_caption_val.get(test_sample.question_id, None)###
      ...
      #test_sample_captions_w_knowledge.append(pc_caption_test)
      ...
      #test_sample_captions = list(test_sample_captions)###
      #test_sample_captions.append(pc_caption_test)###
      #test_sample_captions.extend(test_sample_captions_w_knowledge)###
      ...
      knowledge_caption_test = test_sample_captions_w_knowledge[0]

      # if knowledge_caption_test == pc_caption_test:
      #    used_normal_caption += 1
      ...
      for j,caption in enumerate(test_sample_captions[:NO_OF_CAPTIONS_AS_CONTEXT]):
        ...
        if j < NO_OF_CAPTIONS_AS_CONTEXT-1:
          ...
          prompt += '%s\n%s\nEnd of Context\n'%(caption, pc_caption_test)###
          #prompt += '%s\n%s\n%s\nEnd of Context\n'%(caption, test_sample_captions_w_knowledge[0], pc_caption_test)###
          #prompt += '%s\nEnd of Context\n'%caption
...
def test_in_context_learning_a_ok_vqa_with_beit(llama_model,
  ...
  with open('/scratch/project_462000472/pyry/prophet/PromptCAP/aokvqa_val_captions.json', 'r') as file:
      pc_captions_val = json.load(file)
      qid_to_caption_train.update({entry["question_id"]: entry["pred_caption"] for entry in pc_captions_val})

  # Load training captions
  with open('/scratch/project_462000472/pyry/prophet/PromptCAP/aokvqa_train_captions.json', 'r') as file:
      pc_captions_train = json.load(file)
      qid_to_caption_train.update({entry["question_id"]: entry["caption"] for entry in pc_captions_train})


  with open('/scratch/project_462000472/pyry/prophet/PromptCAP/aokvqa_test_captions.json', 'r') as file:
            pc_captions_test = json.load(file)
  
  qid_to_caption_test = {entry["question_id"]: entry["caption"] for entry in pc_captions_test}
  ...
  for i in tqdm(range(test_annotations_df.shape[0])):
    ...
    for k in range(k_ensemble): # we use k promts for each test sample
      ...
      pc_caption_test = qid_to_caption_test.get(test_sample.question_id, None)###
      test_sample_captions_w_knowledge = [c["caption"] for c in test_sample_captions_w_knowledge]
      #test_sample_captions_w_knowledge.append(pc_caption_test)
      test_sample_captions_w_knowledge, cos_scores = sort_captions_based_on_similarity(test_sample_captions_w_knowledge,raw_image=raw_test_image,model=blip_model,processor=blip_processor,device = device, ascending = False)
      ...
      for j,caption in enumerate(test_sample_captions[:NO_OF_CAPTIONS_AS_CONTEXT]):
        ...
        if j < NO_OF_CAPTIONS_AS_CONTEXT-1:
          ...
          prompt += '%s\nEnd of Context\n'%(caption)
          #prompt += '%s\n%s\n%s\nEnd of Context\n'%(caption, test_sample_captions_w_knowledge[0], pc_caption_test)###
          #prompt += '%s\n%s\nEnd of Context\n'%(caption, knowledge_caption_test)###
      prompt += 'Question: %s\nAnswer:'%test_sample.question
